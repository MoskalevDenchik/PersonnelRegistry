...
namespace DM.PR.Data.Core.Converters.Implement
{
    ...
    {
        public IEnumerable<Employee> ConvertToList(DataSet dataSet) => dataSet.Tables[0].AsEnumerable().Select(empl => new Employee
        {
        ...
        {
            return tables[0].AsEnumerable().Where(dep => dep.Field<int>("Id") == departmentId).Select(dep => new Department
            {
                Id = dep.Field<int>("Id"),
